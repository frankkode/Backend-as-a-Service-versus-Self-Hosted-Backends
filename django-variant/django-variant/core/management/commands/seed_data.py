import sys
# Docker's build context for django-variant/ doesn't include the sibling shared/ folder, so this
# can't be reached by relative path inside the container — it's mounted explicitly instead via the
# `volumes: ["../shared:/shared:ro"]` line in docker-compose.yml (§4.7), landing at this fixed path.
